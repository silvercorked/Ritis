...
namespace engine {
	...
	std::unique_ptr<Model> createCubeModelIndexed(Device& device, glm::vec3 offset) {
		Model::Builder modelBuilder{};
		modelBuilder.vertices = {
			// left face (white)
			{{-.5f, -.5f, -.5f}, {.9f, .9f, .9f}},
			...
			{{.5f, .5f, -0.5f}, {.1f, .8f, .1f}},
			{{-.5f, .5f, -0.5f}, {.1f, .8f, .1f}},
			{{.5f, -.5f, -0.5f}, {.1f, .8f, .1f}}, // still have a lot of vertices cause all 6 faces have a color
		};
		for (auto& v : modelBuilder.vertices) {
			v.position += offset;
		}
	...
	auto FirstApp::loadGameObjects() -> void {
		std::shared_ptr<Model> model = createCubeModelIndexed(this->device, { 0.0f, 0.0f, 0.0f });
